...
# 1. 读入数据，计算 Score
data = read.csv('./dotplot_resources/Prostate_scale.csv', row.names = 1)
data$Score = pmax(pmin(data[[var]], 1.75), -1.75)



data$Pathology <- as.character(data$Pathology)
data$Pathology[data$Pathology == "Stroma & Partially Atrophic Changes"] <- "Stroma & Partially\nAtrophic Changes"
data$Pathology <- factor(data$Pathology, levels = unique(data$Pathology))
...
pdf('./Figure_5F.pdf', width=12, height=4)
print(p)
dev.off()