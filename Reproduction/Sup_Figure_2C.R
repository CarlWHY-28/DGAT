...
data = read.csv('./dotplot_resources/iBreast_scale.csv', row.names = 1)
data$Score = pmax(pmin(data[[var]], 1.75), -1.75)
data$Pathology = factor(data$Pathology,
                        levels = c('tumor','fibrous','immune','fat','necrosis'))
...
pdf('./Sup_Figure_2C.pdf', width=10, height=3.5)
print(p)
dev.off()